...
# Source is two levels up from install/deb-qt5/
SOURCE_DIR="${1:-$(cd "$SCRIPT_DIR/../.." && pwd)}"
VERSION="1.5.0"
PACKAGE_NAME="gemini-commander"
BUILD_DIR="/tmp/${PACKAGE_NAME}-build-qt5"

echo "=========================================="
echo "Gemini Commander .deb Package Builder"
echo "        (Qt5/KF5 version)"
echo "=========================================="
...
install_build_deps() {
    echo ""
    echo "Installing build dependencies (Qt5/KF5)..."
    sudo apt-get update
...
prepare_source() {
    ...
    # Copy debian directory from install/deb-qt5/
    if [ ! -d "$BUILD_DIR/${PACKAGE_NAME}-${VERSION}/debian" ]; then
        ...
        elif [ -d "$SOURCE_DIR/install/deb-qt5/debian" ]; then
            cp -r "$SOURCE_DIR/install/deb-qt5/debian" "$BUILD_DIR/${PACKAGE_NAME}-${VERSION}/"
        else
...
build_package() {
    echo ""
    echo "Building .deb package (Qt5/KF5)..."
