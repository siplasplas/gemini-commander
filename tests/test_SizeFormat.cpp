...
using SizeFormat::formatWithPrefix;
...
TEST(SizeFormatTest, MaxSizeBinaryEi)
{
  ...
  EXPECT_EQ("16.0 Ei", formatWithPrefix(maxValue, /*binaryPrefix=*/true));
}
...
TEST(SizeFormatTest, MaxSizeDecimalE)
{
  ...
  EXPECT_EQ("18.4 E", formatWithPrefix(maxValue, /*binaryPrefix=*/false));
}

TEST(SizeFormatTest, KiloExamples)
{
  // 1020 / 1000 = 1.02  -> "1.02 k"
  EXPECT_EQ("1.02 k", formatWithPrefix(1020u, /*binaryPrefix=*/false));

  // 1020 < 1024, so it is left without a binary prefix
  EXPECT_EQ("1020", formatWithPrefix(1020u, /*binaryPrefix=*/true));
}

TEST(SizeFormatTest, SmallValuesFormatting)
{
  // Kilka sanity-checków, jeśli implementacja tak działa
  EXPECT_EQ("1",   formatWithPrefix(1u,   false));
  EXPECT_EQ("999", formatWithPrefix(999u, false));
  EXPECT_EQ("1.02 k", formatWithPrefix(1020u, false));
  EXPECT_EQ("1020", formatWithPrefix(1020u, true));
}
